...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while roll_times <= num_rolls:
        original_points = dice()
        #if there is "pigout"
        if original_points == 1:
            pigout_times += 1
        #counting
        final_points = original_points + final_points
        roll_times += 1
    #count points 
    if pigout_times != 0:
        return 1
...
def is_swap(player_score, opponent_score):
    ...
    if opponent_score >= 10:
        ...
        if abs((player_score % 10) - (opponent_score % 10)) == 0:
            return True
    return False
    # END PROBLEM 4
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    # BEGIN PROBLEM 6
        "*** YOUR CODE HERE ***"
        if turn == 0:
            commtary = say(score0,score1)
        else:
            commtary = commtary(score0,score1)
        turn = turn + 1
    # END PROBLEM 6
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    def judge_running_high(this_score0,this_score1):
        if who == 0:
            judge_score = this_score0 - last_score
            if judge_score > running_high:
                print(judge_score, "point(s)! That's the biggest gain yet for Player", who)
                return announce_highest(who,this_score0,judge_score)
            else:
                return announce_highest(who,this_score0,running_high)
        else:
            judge_score = this_score1 - last_score
            if judge_score > running_high:
                print(judge_score, "point(s)! That's the biggest gain yet for Player", who)
                return announce_highest(who,this_score1,judge_score)
            else:
                return announce_highest(who,this_score1,running_high)
    return judge_running_high
    # END PROBLEM 7
...
def make_averaged(original_function, trials_count=1000):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    # args是随机数字 trials_count是调用函数的次数 每一次original——function（args）返回一个值 res保存求和  res / trials——count计算出最终结果
